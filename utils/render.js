...
const renderPublic = (filePath, response) => {
  ...
  const splittedPath = splitPath(filePath);
  const contentType = getContentType(splittedPath[1]);
...
/**
 * Gets full filepath of the file
 * 
...
/**
 * Splits the path between the filename and the rest
 * 
 * @param {*} filePath the filepath
 * @returns {*} array where the first element is the filename and the second element is the rest
 */
